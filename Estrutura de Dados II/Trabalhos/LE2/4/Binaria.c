...
#include <stdlib.h>
#include <time.h>
#include "Binaria.h" // Inclui protótipos
...
// Funcao para escolher um valor aleatorio que existe na arvore
void get_value(int *array, int size, int *value){

    int random_value = rand()%size;

    *value = array[random_value];
}
...
int consulta_ArvBin_Recurs(ArvBin* raiz, int valor)
{
    ...
    if ((*raiz)->info == valor) {
        ...
        return 1; // Retorna 1 para indicar que o valor foi encontrado
    }

    if(valor > (*raiz)->info)
    {
        return consulta_ArvBin_Recurs(&((*raiz)->dir), valor);
    }
    else
       return consulta_ArvBin_Recurs(&((*raiz)->esq), valor);
}
