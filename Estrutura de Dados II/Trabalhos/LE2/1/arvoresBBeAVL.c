#include <stdio.h>
#include <stdlib.h>
#include "ArvoresBBeAVL.h" // Inclui protótipos

// A estrutura do nó da árvore, possui 4 campos: informação inteira, altura inteira, ponteiro esquerdo e ponteiro direito.
struct NO{
   int info;
   int alt;
   struct NO* esq;
...
void RotacaoLL (ArvAVL *A) {
   struct NO *B; // recebe o nó à esquerda do nó raiz
   B = (*A)->esq;

   // reconexões correpondentes à rotação à direita
   (*A)->esq = B->dir;
   B->dir = *A;
...
int insere_ArvAVL(ArvAVL* raiz, int valor){
   int res; //res serve p pegar as respostas da inserção ao longo das chamadas
   // caso trivial ou nulo
   if (raiz == NULL)
      return 0;
...
struct NO* procuraMenor(struct NO* atual){  //trata o caso com 2 filhos
    struct NO *no1 = atual, *no2 = atual->esq;
    while (no2 != NULL){
        no1 = no2;      //vai trocando e descendo pela esquerda até encontrar o menor valor
        no2 = no2->esq;
    }
    return no1; //retorna o menor valor encontrado
}
...
int remove_ArvAVL(ArvAVL* raiz, int valor){     //remover nó a direita é o mesmo que inserir um nó a esquerda
   int res;
   // caso trivial ou nulo
   if (raiz == NULL)
      return 0;
...
void libera_ArvBin(ArvBin* raiz){
   ...
   if (raiz == NULL) /* caso a árvore não tenha sido criada */
     return;
   // chamada a função recursiva
   libera_NO2(*raiz); /* libera cada nó */
   // libera a raiz
   free(raiz);
};
...
struct NO2* remove_atual(struct NO2* atual){
   struct NO2 *no1, *no2; /* ponteiros esquerdo e direito de atual */
   ...
   //copia o filho mais a direita na sub-arvore esquerda para o lugar do nó removido
...
int consulta_ArvBin(ArvBin* raiz, int valor){
   ...
   if (raiz == NULL)
       return 0;
   // percurso sequêncial
   struct NO2* atual = *raiz;   /* ponteiro ao nó raiz */
   while (atual != NULL){
      if (valor == atual->info){
         return 1;             /* valor encontrado */
      }
