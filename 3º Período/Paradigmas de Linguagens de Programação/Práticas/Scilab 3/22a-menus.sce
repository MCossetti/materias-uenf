...
discip = ['Paradigmas de Linguagens','Engenharia de Software','Computação Gráfica','Redes sem Fio'];
 ...
 professor = ['Ausberto', 'Annabell', 'Tang', 'Rivera'];
 universidade = ['UENF','IFF', 'UFF'];

L1=list('Disciplina',1, discip); //titulo, Numero opção default, lista-opcoes
L2=list('Dia',1, dia); 
L3=list('Horario',1,turno); 
L4=list('Professor', 1, professor);
L5=list('Universidade', 1, universidade)

OpMatr=x_choices([Aluno;' Escolha UMA disciplina para matricular:'],list(L1, L2, L3, L4, L5)); 
...
'      Universidade : '+ string(universidade(OpMatr(5)))]);
